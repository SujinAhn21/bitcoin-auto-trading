...
# pyupbit을 사용하여 10일간의 데이터 가져오기
ticker = "KRW-BTC"                                  # 10일치
df = pyupbit.get_ohlcv(ticker, interval="minute60", count=24*10)

# 데이터 정규화
scaler = MinMaxScaler(feature_range=(0, 1))
df_scaled = scaler.fit_transform(df)

# LSTM에 사용할 데이터를 준비하는 함수
...
# LSTM에 사용할 시간 스텝 수 선택
n_steps = 10
X, y = prepare_data(df_scaled, n_steps)
...
model.add(Dense(df.shape[1]))  # 출력 레이어에는 활성화 함수를 적용하지 않음
model.compile(optimizer='adam', loss='mse')  # 평균 제곱 오차를 사용

# 모델 훈련
model.fit(X, y, epochs=50, batch_size=32, verbose=1)

# 모델을 사용하여 다음 24시간 동안의 가격 예측
last_10_days_data = df_scaled[-n_steps:]
last_10_days_data = np.reshape(last_10_days_data, (1, n_steps, df.shape[1]))
prediction = model.predict(last_10_days_data)

# 예측값을 정규화된 값에서 실제 값으로 변환
predicted_prices = scaler.inverse_transform(prediction.reshape(-1, df.shape[1]))

# LSTM으로 예측한 가장 최근의 예측값
LSTM_predicted_price = predicted_prices[0][-1]

# <설명>
# 예측된 가격과 현재 가격 비교
# predicted_prices[0]: 모델에 의해 예측된 다음 24시간 동안의 가격 데이터 배열을 나타냄
# [-1]: 배열의 마지막 값을 선택함
# 따라서 predicted_prices[0][-1]은 모델이 예측한 가격 중에서 가장 마지막(최신) 값을 나타냄. 
# 이 값은 시간이 흐른 후의 예측값 중 가장 최근에 해당하는 가격임. 
# ex) 예를 들어, 모델이 24시간 동안의 가격을 예측하였고 현재 시간이 0시이면, 
# predicted_prices[0][-1]은 모델이 예측한 24시간 후의 가격을 나타냄.
...
while True:
    try:
        ...
        if start_time <= now < (end_time - datetime.timedelta(seconds=10)):
            ...
            # 3) 매수 조건으로 현재 가격이 LSTM으로 예측한 가격보다 높은 경우로 설정
            # => 상승장 예측
            if target_price >= current_price and current_price >= ma5 and current_price >= LSTM_predicted_price:
                krw = get_balance("KRW")
                if krw >= 5000:
